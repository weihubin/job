...
@RestController
@CrossOrigin
public class QryTradingAccController extends BaseController {
    private final static Logger logger = LoggerFactory.getLogger(QryTradingAccController.class);
    @Autowired
    private QryTradingAccService qryTradingAccService;

    @RequestMapping(value = URL.REQ_QRY_TRADING_ACC, method = RequestMethod.POST)
    public ResultDto<?> qryTradingAcc(@RequestBody String msg) {
        try {
            return qryTradingAccService.qryTradingAcc(msg);
